...
for i in range(0,55):
    ...
    query.accept('netcdf4')
    query.variables('Convective_available_potential_energy_surface','Precipitation_rate_surface','Pressure_reduced_to_MSL_msl','Vertical_velocity_pressure_isobaric','Relative_humidity_isobaric','Temperature_isobaric','u-component_of_wind_isobaric','v-component_of_wind_isobaric','Geopotential_height_isobaric')
    ...
    capep = ax.contourf(x, y, CAPE, levels=[100, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2500, 3000, 3500, 4000, 4500, 5000], alpha = 0.5, cmap='RdPu')#['#0099ff00', '#4066ffb3', '#8066ff8c', '#BF66ff66','#8cff66','#b3ff66','#d9ff66','#ffff66','#ffd966','#ffcc66','#ffb366','#ff8c66','#ff6666','#ff668c','#ff66b3','#ff66d9','#ff66ff'])
    ...
    ax.set_title('Synoptic Composite Valid at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item(), fontsize=36)
    ...
    plt.savefig(output_dir+'/synoptic/synoptic_composite'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
