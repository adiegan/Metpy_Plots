...
mkdir_p(output_dir)
mkdir_p(output_dir+'/H300_qvec')

#print(mkdir_p(output_dir+'/H300_qvec'))
...
for i in range(0,41):
    ...
    zH85 = data['height'].metpy.sel(vertical=300 * units.hPa)
    ...
    #Get heights array
    heights = data['height'].metpy.loc[{'time': time[0], 'vertical': 300. * units.hPa}]
    ...
    temp_ = data['temperature'].metpy.sel(vertical=300*units.hPa).squeeze()
    #temp_ = mpcalc.smooth_n_point['temperature'].metpy.sel(vertical=850*units.hPa).squeeze(), 9, 50

    u_wind = data['u'].metpy.sel(vertical=300*units.hPa).squeeze()
    v_wind = data['v'].metpy.sel(vertical=300*units.hPa).squeeze()

    uqvect, vqvect = mpcalc.q_vector(u_wind, v_wind, temp_, 300*units.hPa, dx, dy)
    ...
    height_contour = data['height'].metpy.sel(vertical=300*units.hPa).squeeze()
    temp_contour = data['temperature'].metpy.sel(vertical=300*units.hPa).squeeze()
    h_contour = ax.contour(x, y, height_contour, colors='k', levels=range(0, 13000, 120))
    h_contour.clabel(fontsize=8, colors='k', inline=1, inline_spacing=8, fmt='%i', rightside_up=True, use_clabeltext=True)
    t_contour = ax.contour(x, y, temp_contour, colors='xkcd:light red', levels=range(-80, 20, 10), alpha=0.8, linestyles='--')
    ...
    ax.set_title('300 hPa Heights (m), Temperature (\u00B0C), and Q-Vector Divergence at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item())
    plt.savefig(output_dir+'/H300_qvec/height_temp_qvec3_'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
