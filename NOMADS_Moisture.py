...
#import matplotlib.gridatapec as gridatapec
...
data = xr.open_dataset('http://nomads.ncep.noaa.gov:80/dods/gfs_0p25/gfs'+mdate+'/gfs_0p25_'+get_init_hr(hour)+'z')
...
for i in range(0,120):
    ...
    data = data.metpy.assign_crs(grid_mapping_name='latitude_longitude')
    # Select desired vars
    datavars = data[['hgtprs', 'tmpprs', 'ugrdprs', 'vgrdprs', 'rhprs']]


	# Select time
    #data = data.sel(time=data.time[0])

	# Select level
    data = data.sel(lev=[925, 700])
    ...
    cs2 = ax.contour(data.lon, data.lat, T, clev, colors='blue', linewidths=1.25, linestyles='--', transform=datacrs)
    plt.clabel(cs2, fontsize=10, inline=1, inline_spacing=10, fmt='%i', rightside_up=True, use_clabeltext=True)

	# Plot Temperature Contours (>=0)
    clev = np.arange(0, 41, 2)
    cs2 = ax.contour(data.lon, data.lat, T, clev, colors='red', linewidths=1.25, linestyles='--', transform=datacrs)
    plt.clabel(cs2, fontsize=10, inline=1, inline_spacing=10, fmt='%i', rightside_up=True, use_clabeltext=True)

	# Plot Colorfill of RH
    cint = np.arange(70, 101, 2)
    cf = ax.contourf(data.lon, data.lat, data.rhprs, cint[cint != 0], extend='max', cmap='Greens', transform=datacrs)
	...
	# Add plot headers
    plt.title(f'GFS 925-700mb Mean RH, Temp, Height, and Wind', loc='left')
    plt.title(f'Run: {valid.strftime("%a %Y-%m-%d %H:%M")} UTC\nValid: {valid.strftime("%a %Y-%m-%d %H:%M")} UTC', loc='right')

    # Export plot and close
    #plt.show()
    ######## Save the plot
    plt.savefig(output_dir+'/GFS_para_TyS/gfs_moisture_'+time.dt.strftime('%Y-%m-%d %H%M').item()+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()